...
const TeamSection: React.FC = () => {
  return (
    <section id="time" className="py-24 bg-transparent transition-colors">
      <div className="container mx-auto px-6">
        <div className="text-center mb-20">
          <p className="text-gold text-[10px] font-bold uppercase tracking-[0.4em] mb-4">Board Executivo</p>
          <h2 className="text-5xl font-serif text-navy-dark dark:text-white">Nosso <span className="text-gold italic">Time de Especialistas</span></h2>
            ...
            <div key={i} className="group text-center">
              <div className="relative mb-8 inline-block">
                ...
                <img 
                  src={member.image} 
                  alt={member.name} 
                  className="w-48 h-48 rounded-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 border-4 border-slate-50 dark:border-white/5 shadow-2xl relative z-10"
                />
              </div>
              <h4 className="text-xl font-serif text-navy-dark dark:text-white mb-2">{member.name}</h4>
              <p className="text-gold text-[10px] font-black uppercase tracking-widest mb-4">{member.role}</p>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-tighter mb-4 border-b border-slate-100 dark:border-white/5 pb-4 inline-block">{member.specialty}</p>
              <p className="text-slate-500 dark:text-slate-400 text-sm font-light leading-relaxed max-w-xs mx-auto">
                {member.bio}
              </p>
            </div>
          ))}
        </div>
        <div className="mt-20 p-8 rounded-sm flex flex-wrap justify-center gap-12 bg-transparent">
