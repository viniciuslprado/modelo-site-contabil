    ...
    <section className="py-24 bg-white dark:bg-navy-dark transition-colors">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <h2 className="text-4xl font-serif mb-4 dark:text-white">Soluções por <span className="text-gold italic">Nicho</span></h2>
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {NICHES.map((n) => (
            <button
              key={n.id}
              onClick={() => setActive(n)}
              className={`flex items-center gap-3 px-6 py-3 rounded-full border-2 transition-all duration-300 font-bold text-sm ${active.id === n.id ? 'bg-gold border-gold text-navy-dark' : 'border-slate-100 dark:border-white/10 text-slate-400 hover:border-gold/50'}`}
            >
              {n.icon}
              {n.label}
            </button>
        ...
        <div className="max-w-4xl mx-auto bg-slate-50 dark:bg-white/5 rounded-sm p-12 border border-slate-100 dark:border-white/10 animate-[fadeIn_0.5s_ease-out]">
          <div className="flex flex-col md:flex-row gap-12 items-center">
            <div className="flex-1">
              <h3 className="text-2xl font-serif text-navy-dark dark:text-white mb-4">{active.title}</h3>
              <p className="text-slate-500 dark:text-slate-400 mb-6">{active.description}</p>
            </div>
          </div>
        </div>
      </div>
