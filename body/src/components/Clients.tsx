		...
		<section id="clientes" className="py-24">
			<div className="container-custom">
				...
				>
					...
					<h2 className="section-title mt-2 text-primary font-bold text-3xl">
						Empresas que <span className="text-gold">confiam em nós</span>
					</h2>
					<p className="section-subtitle mx-auto mt-4 text-secondary-foreground/90 text-lg font-medium">
						Temos orgulho de ser parceiros de empresas de diversos segmentos,
						contribuindo para seu crescimento e sucesso
					</p>
				</motion.div>

				<Carousel opts={{ loop: true, align: "start", dragFree: true }} className="mb-20">
					<CarouselContent>
						{clientLogos.map((client, index) => (
							<CarouselItem key={index} className="basis-1/2 md:basis-1/4 px-2">
								<div className="group flex flex-col items-center justify-center h-32 bg-muted rounded-lg border border-border hover:border-primary/30 hover:shadow-lg transition-all duration-300">
									<div className="w-16 h-16 mx-auto bg-secondary/80 rounded-lg flex items-center justify-center group-hover:bg-primary/20 transition-colors">
										<span className="text-2xl font-display font-bold text-muted-foreground group-hover:text-primary transition-colors">
											{client.initials}
										</span>
									</div>
									<span className="text-sm text-muted-foreground mt-2 block opacity-0 group-hover:opacity-100 transition-opacity">
										{client.name}
									</span>
								</div>
							</CarouselItem>
